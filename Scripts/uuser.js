    ...
    if ($("#passOld").length) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "1") {
                            ...
                            window.setTimeout("window.location.href = '/Home/Index';", 500);
                        }
                        else {
        ...
        if ($('#datatable-user').length) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == "1") {
                                ...
                                toastr["info"]("Bạn không được xóa chính tài khoản của bạn đang đăng nhập.", "Thông báo");
                            }
                        }
                    })
                }
            ...
            $("button[name='showOrHidePass']").click(function () {
                ...
                $(this).parent().parent().find("input")[0].type = (className == "glyphicon glyphicon-eye-open") ? "text" : "password";
                span.className = (className == "glyphicon glyphicon-eye-open") ? "glyphicon glyphicon-eye-close" : "glyphicon glyphicon-eye-open";
                $(this).attr("class", (className == "glyphicon glyphicon-eye-open") ? "btn btn-primary" : "btn btn-success");
            });
            $("#cancelbutton").click(function () {
                if (confirm("Bạn có chắc hủy. Mọi thao tác của bạn sẽ không được lưu lại.")) {
            ...
            $("#sumbitbutton").click(function () {
                if (validator.checkAll($("form"))) {
                    if ($("#passAgain").val() != $("#pass").val()) {
                        $("#pass").focus();
                        toastr["error"]("Mật khẩu nhập lại không trùng với mật khẩu.", "Lỗi");
                        return;
                    }
                    var id = $("#userId").val();
