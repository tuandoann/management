            ...
            if (confirm("Bạn có muốn xóa tất cả phần tử được chọn!")) {
                ...
                if(itemArray.length >0)
                {
                    ...
                    $.each(itemArray, function (index, value) {
                        lsIdItem.push(value.DepartmentID);
                    });
                    $.ajax({
                        url: '/Department/deleteMany',
                        type: 'post',
                        datatype: 'json',
        ...
        $("#sumbitbutton").click(function () {
            if (validator.checkAll($("form"))) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "0") {
                            toastr["error"]("Phòng ban lưu thất bại.", "Thông báo")
                            flag = false;
                        }
                        else {
                            toastr["success"]("Phòng ban lưu thành công.", "Thông báo")
                            window.location.href = "/Department/Index";
                        }
                    }
                })
